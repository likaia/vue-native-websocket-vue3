...
export default class {
    ...
    private readonly passToStoreHandler; // 传输数据时的处理函数 | Processing function when transferring data
    private readonly store; // 启用vuex时传入vuex的store | Pass in vuex store when vuex is enabled
    private readonly mutations; // 启用vuex时传入vuex中的mutations | Pass in the mutations in vuex when vuex is enabled
